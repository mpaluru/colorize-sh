...
for _nfg in ${NORMAL_FG_COLORS_LC}; do
    echo "${_COLORS_PREFIX_LC_}print_${_nfg} \"${_COLORS_PREFIX_LC_}print_${_nfg}\""
    echo "printf \"\n\""
done
echo ""

for _bfg in ${BOLD_FG_COLORS_LC}; do
    echo "${_COLORS_PREFIX_LC_}print_${_bfg} \"${_COLORS_PREFIX_LC_}print_${_bfg}\""
    echo "printf \"\n\""
done
echo ""
...
for _fg in ${ALL_FG_COLORS}; do
    ...
    for _bg in ${BG_COLORS_LC}; do
        echo "${_COLORS_PREFIX_LC_}print_${_fg}_${_bg} \"${_COLORS_PREFIX_LC_}print_${_fg}_${_bg}\""
        echo "printf \"\n\""
    done
done
